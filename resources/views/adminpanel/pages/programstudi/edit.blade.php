...
{{-- End Toast or alert--}}
    ...
    <div class="p-4 mx-8 bg-white rounded-bl-lg rounded-br-lg">
        <form action="{{ url('admin') }}/prodi/detail/{{ request()->segment(4) }}" method="post" enctype="multipart/form-data" id="form-edit-prodi">
            @csrf
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        ...
                        <input type="text" id="nama_prodi" name="nama_prodi"
                            ...
                            required value="{{ $dataprodi->judul, old('nama_prodi') }}">
                    </div>
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-900" for="file_input">Logo</label>
                        <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none" id="file_input" type="file" name="logo">
                    </div>
                    <div>
                        <label for="singkatan" class="block mb-2 text-sm font-medium text-gray-900">Singkatan</label>
                        <input type="text" id="singkatan" name="singkatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required value="{{ $dataprodi->sinonim, old('singkatan') }}">
                    </div>
                    <div>
                        <label for="ketua_jurusan" class="block mb-2 text-sm font-medium text-gray-900">Ketua
                            Jurusan</label>
                        <select id="ketua_jurusan" name="ketua_jurusan"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option selected>--Pilih Ketua Jurusan--</option>
                            @foreach ($datakajur as $kajur)
                                <option value="{{ $kajur->id }}" {{ $dataprodi->kajurid == $kajur->id ? 'selected' : '' }} >{{ $kajur->nama }} | {{ $kajur->jabatan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="deskripsi" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" rows="6" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500">{{ $dataprodi->description, old('deskripsi') }}</textarea>
                    </div>
                </div>
            </div>
                        ...
                        <div class="flex">
                            <div class="flex-1">
                                ...
                                <input type="text" id="nama_prestasi"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    >
                                    <small class="text-yellow-800" id="information-nama-prestasi"></small>
                            </div>
                            <div class="pt-7">
                        ...
                        <div class="flex">
                            <div class="flex-1">
                                ...
                                <input type="text" id="nama_pekerjaan"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    >
                                    <small class="text-yellow-800" id="information-nama-pekerjaan"></small>
                            </div>
                            <div class="pt-7">
                                <button type="button"
                                    class="p-2.5 px-3 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300" onclick="addPekerjaan()">
                                    <i class="bi bi-plus-lg"></i>
                                </button>
                            </div>
                        </div>
                ...
                <div class="flex flex-wrap gap-16">
                    <div class="w-full overflow-y-auto sm:w-1/2">
                        <table class="w-full text-left">
                            ...
                            <tbody class="border-b border-slate-300" id="tb-mapel-ajar">
                        ...
                        <div class="flex">
                            <div class="flex-1">
                                <label for="nama_mapel_ajar" class="block mb-2 text-sm font-medium text-gray-900">Mapel di
                                    Pelajari</label>
                                <input type="text" id="nama_mapel_ajar"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                    >
                                    <small class="text-yellow-800" id="information-nama-mapel-ajar"></small>
                            </div>
                            <div class="pt-7">
                                <button type="button"
                                    class="p-2.5 px-3 ml-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300" onclick="addMapelAjar()">
                                    <i class="bi bi-plus-lg"></i>
                                </button>
                            </div>
                        </div>
...
@endsection
@push('jsexternal')
    <script src="/jsadmin/prodi/edit.js"></script>
@endpush
