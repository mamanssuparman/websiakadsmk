...
{{-- End Toast or alert--}}
    ...
    <div class="p-4 mx-8 bg-white rounded-bl-lg rounded-br-lg">
        <form action="{{ url('admin') }}/profile/edit/{{ base64_encode($dataprofile->id) }}" method="POST">
            @csrf
            <div class="flex flex-row w-full px-6">
                <div class="flex flex-col w-full pb-2">
                    ...
                    <div class="pb-2">
                        <input type="text" name="judulprofile" id="" class="w-full h-8 rounded-md border-slate-300" value="{{ $dataprofile->judul, old('judulprofile') }}">
                        @error('judulprofile')
                    ...
                    <div>
                        <textarea name="editor1" id="description" cols="20" rows="10" class="ckeditor">
                            {{ $dataprofile->description, old('editor1') }}
                        </textarea>
                        @error('editor1')
                            <small class="italic text-yellow-800">{{ $message }}</small>
                        @enderror
